...
import config_old
from dataset import CUDAPrefetcher, TrainValidImageDataset, TestImageDataset, TrainValidImageDataset_PASCAL
from image_quality_assessment import PSNR, SSIM
from model import Discriminator, Generator, ContentLoss, Generator_X8, count_parameters,Generator_X6,Generator_X12
...
def main():
    ...
    results_dir = os.path.join("E:/jingou/ESRGAN/save_weights", config.exp_name)
    ...
    for epoch in range(config.start_epoch, config.epochs):
        ...
        if is_best:
            torch.save({"epoch": epoch + 1,
                        ...
                        "optimizer": d_optimizer.state_dict(),
                        "scheduler": d_scheduler.state_dict()},
                       os.path.join(results_dir, f"d_best.pth.tar"))
            torch.save({"epoch": epoch + 1,
                        "best_psnr": best_psnr,
                        "best_ssim": best_ssim,
                        "state_dict": generator.state_dict(),
        ...
        if (epoch + 1) == config.epochs:
            torch.save({"epoch": epoch + 1,
                        ...
                        "optimizer": d_optimizer.state_dict(),
                        "scheduler": d_scheduler.state_dict()},
                       os.path.join(results_dir, f"d_last.pth.tar"))
            torch.save({"epoch": epoch + 1,
                        "best_psnr": best_psnr,
                        "best_ssim": best_ssim,
                        "state_dict": generator.state_dict(),
...
def load_dataset() -> [CUDAPrefetcher, CUDAPrefetcher, CUDAPrefetcher]:
    ...
    train_datasets = TrainValidImageDataset_PASCAL(config.train_image_dir, config.image_size, config.upscale_factor,
                                                   config.train_txt_name, "Train")
    valid_datasets = TrainValidImageDataset_PASCAL(config.valid_image_dir, config.image_size, config.upscale_factor,
                                                   config.valid_txt_name, "Valid")
    test_datasets = TestImageDataset(config.test_lr_image_dir, config.test_hr_image_dir)
