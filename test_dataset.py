...
import config_old
from dataset import CUDAPrefetcher, TrainValidImageDataset, TestImageDataset,TrainValidVOCDataset
...
def main():
    ...
    train_prefetcher, valid_prefetcher, test_prefetcher = load_dataset_VOC()
    ...
    mask = batch_data["semseg"]
    label = batch_data["label"]
    info = batch_data["meta"]
    print(lr.shape,hr.shape,image.shape,mask.shape,label,info)
...
def load_dataset_VOC() -> [CUDAPrefetcher, CUDAPrefetcher, CUDAPrefetcher]:
    # Load train, test and valid datasets
    train_datasets = TrainValidVOCDataset(voc_root='D:/jingou/VOC/VOCtrainval_11-May-2012',txt_name='train.txt')
    valid_datasets = TrainValidVOCDataset(voc_root='D:/jingou/VOC/VOCtrainval_11-May-2012',txt_name='val.txt')
    test_datasets = TestImageDataset(config.test_lr_image_dir, config.test_hr_image_dir)
